...
function SyntaxHighlightedCode(props) {
  ...
  useEffect(() => {
    if (ref.current && props.className?.includes('lang-') && window.hljs) {
      ...
      // Remove the attribute so hljs will reprocess if needed.
    ...
    receiveMessage('project-message', (data) => {
      ...
      if (data.sender._id === 'ai') {
        ...
        if (webContainer && typeof webContainer.mount === 'function') {
          ...
          console.error('Web container not available for mounting');
        }
        if (message.fileTree) {
          ...
          <div className="top flex justify-between w-full">
            ...
            <div className="actions flex gap-2">
              <button
                onClick={async () => {
                  if (!webContainer || typeof webContainer.mount !== 'function') {
                    console.error('Web container is not available or mount method is missing.');
                    return;
                  }
                  try {
                    // Mount the file tree into the container.
